...
**Implications:**
- **`networks.md §2`:** add a logistics channel-capacity model mirroring §3 power — `LogisticsNetworkMembers.channel_capacity()` / `channels_in_use()`, the per-port consumption rule, cable-tier channel ratings, and the non-destructive over-budget behavior (ports past the budget go unserved, no cable/machine damage), plus segmentation via controller/interface boundaries.
- **Content:** the Channel Upgrade / Logistics Controller / Sub-network Router nodes (`standard-run-design.md §5`) get authored with **channel-count** ratings per cable tier (numbers TBD in a content pass), not flow rates. New cable-tier items gate on Engineering research.
