...
## 2026-07-14 — Dissolved `standard-run-design.md`; balance methodology → `balance.md`

**Decision:** The standalone Standard-run content-design doc was **deleted**. It had become a redundant, more-granular twin of `tech-tree-design.md` (which holds the holistic tier design — base/exotic ratios, unlock-rate targets, tier themes, gates) and its §5 tier tables were the main doc-vs-doc drift source. The content was dissolved as follows: **tier design** → `tech-tree-design.md` (§3 holistic, §6bis authored RON); **specific balance numbers** → the `.ron` assets + the scenario-runner baseline (never restated in prose); **general balance methodology + balance-flavored open questions** → the new hand-written `balance.md`; **byproduct→reactivity** → GDD §11 (already covered); **escape / second-site** → `technical/escape-condition.md §7.1`; **research ladder** → `technical/research.md §3`. Machine-dedication (§3.3) was already a shipped design-decision (config modules, 2026-07-10). All 13 inbound refs were repointed; the doc is git-tracked so the deletion is reversible.

**Rationale:** Docs are the source of truth, but a doc that re-states numbers and tier content already owned elsewhere is pure sync burden — it drifts silently the moment the RON or `tech-tree-design.md` changes. Removing the twin makes `tech-tree-design.md` + the `.ron` assets the single authority for tier content and quantities, and gives balance *method* its own home. Specific quantities were deliberately left to die into the assets/tests rather than migrated into prose.

**Notes:** `balance.md` added to `docs/README.md`. The current measured baseline per difficulty will live in a separate auto-generated `balance-state.md` (emitted by `scenario balance --emit`, a parallel code task — not created here). Deferred-scope flags from the old §9 (frontier-variant second config; machine-dedication config modules with no code yet) remain tracked in `escape-condition.md §7.1` and this log (2026-07-10) respectively — flagged to project-manager for any tasks.md backlog placement.

---

## 2026-07-14 — Progression is 100% RON-defined; only the victory trigger is hardcoded

**Decision:** All tech-tree progression — which recipes/machines a node unlocks, and how a node is earned — lives entirely in `assets/tech_nodes/*.ron` and is applied by the generic `apply_effects`/`check_research_unlocks` engine (`src/research/mod.rs`). The **only** progression left in Rust is the *victory kernel*: the `EscapeObjective` marker → `escape_objective_system` → `EscapeEvent` chain (`src/escape/mod.rs`), which fires the win when a recipe completes on an escape-objective machine. The stray `unlock_gateway_on_discovery` system (hardcoded: on `gateway_ruins` discovery, insert the `activate_gateway` recipe) was **deleted** and folded into the `gateway_theory` node, which already gates on that same `ExplorationDiscovery("gateway_ruins")` and now also carries `UnlockRecipes([activate_gateway])`.

**Rationale:** Content authors work only in RON via the `exergon-assets` tooling; any unlock hidden in Rust is invisible to them and to the content lint, so it silently drifts. `gateway_theory` firing on the same discovery event made the hardcode pure duplication. Keeping the victory *trigger* hardcoded is deliberate — it's a single engine primitive (a recipe completing on a marked machine), not progression data, and every escape structure routes through it.

**Research currency = item + naming convention (kept, not refactored to a field):** A recipe output `research.<theme>` (or legacy `research_points` → `material`) is credited to the `ResearchPool` under `<theme>` instead of stored, decoded by `research_theme_of`. New point types need only a new `research.<theme>` output + item def — no Rust. We considered promoting this to an explicit `ItemDef` field but chose to keep the convention (less churn; already moddable). The four in-use currencies (`research.{material,engineering,discovery,synthesis}`) now have real item defs for discoverability + lint validity.

**Tooling added:** (1) a whole-tree **content lint** (`tests/content_lint.rs`, in `cargo test`) that rejects dangling/inverted prerequisites, unlocks of undefined recipes/templates/items, and recipes no node can reach — it caught 9 real gaps on introduction (a nonexistent `drone_scan` unlock, the unwired `wash→dust→ingot` smelt recipes, plate-scrap recycling, the whole-metal `draw_metal` template, and the unwired coal-combustion power path). (2) a **balance sweep** subcommand, `scenario balance <file> [--seeds N]`, that runs one baseline across N seeds and flags victory-time / tier-pacing outliers and DNFs (`crates/scenario-runner/src/balance.rs`).

**Notes:** The coal path was completed rather than deleted — a new `combustion_power` (Power, T2) node unlocks the `combustion_generator` machine + `make_combustion_generator` + `combust_coal`. Known still-latent (not lint-caught, machine-side): `wind_power` unlocks a `wind_generator` machine that has no make- or run-recipe. The dead `Observation` unlock vector and unused `exclusive_group` field are kept for later, like before.

---

## 2026-07-14 — Sub-network Router mechanics: cable-boundary + per-side channel + filtered crossings

**Decision:** A Sub-network Router is specified (`networks.md §Sub-network Routers`) as a **non-conducting cable boundary**, not a divider on one shared net. Placing it partitions the connected cable graph into **N distinct `LogisticsNetwork` entities**, each with its own independent channel budget (`channel_capacity()` = min cable tier of *its own* segments). The router owns **one port per side**, each a member of that side's sub-net, and **each side-port consumes 1 channel** on its sub-net — so a 2-sided Router I costs 2 channels total (one per side). Cross-net item movement is **explicitly filtered**: a `RouterFilter` lists `Crossing { item_id, from, to }` entries and `router_transfer_system` relays only those items (`take_items` source → `give_items` dest, no per-tick rate cap). Router ports are priority-pinned so the over-budget shed severs leaf machine ports, never a boundary. Router II is a strict superset of I (multi-sided hubs ≥3, nested sub-nets, higher crossing cap).

**Rationale:** The 2026-07-13 rename settled the *name*; this settles the *behavior* the name implies. Making the router a real graph-partition (not a soft divider) is what gives each sub-net an **independent** budget — the entire payoff of segmentation (split a 40-port/8-channel net into two 20-port sub-nets that each fit). The **per-side channel cost** is the key friction knob: segmentation isn't free, so players cut along low-traffic seams rather than spamming routers to dodge every limit — architecture friction (Pillar-2), not a runtime throttle. **Filtered** (vs transparent-proxy) crossings make cross-net dependencies a *declared* design decision and preserve isolation; a transparent router would leak the very budget separation it exists to create. No flow-rate cap keeps this the item-flow layer's only lever = channels (GDD §10), consistent with the 2026-07-12 channel-capacity decision.

**Alternatives rejected:** *Soft divider on one net* (shared budget, router just tags a boundary) — gives no independent budget, defeats the purpose. *Transparent proxy* (router relays all `has_items`/`take_items` blindly) — leaks isolation, no planning intent, and makes cross-net routing free. *Per-tick throughput cap on crossings* — reintroduces the flow-rate model already rejected twice (2026-07-10, 2026-07-12); meters the clock, not the topology.

**Notes:** Docs-only, unimplemented — depends on the channel-capacity engine work (2026-07-12), same Demo-scope sequencing after the VS playtest gate. Crossing caps (N) and per-tier channel ratings are TBD content. New components `SubnetRouter` / `RouterFilter` and system `router_transfer_system` are design placeholders, not yet in `src/`.

---

## 2026-07-13 — Scenario runner is a prescriptive `steps` list, not fixed Rust choreography

**Decision:** A scenario (`scenarios/*.ron`) is now an ordered **`steps` list** that the step interpreter (`Scenario::run` → `run_steps` → `exec_step`, `crates/scenario-runner`) executes verbatim — one real mechanic per `Step` (`Deploy`/`Place`/`Craft`/`Ensure`/`Research`/`Recon`/`Scan`/`Install`/`Pump`/`Build`). The previous per-difficulty Rust choreographies (`run_standard`, `run_initiation`, `drive_to_victory`) and the data "knobs" they read (`material_nodes`/…/`synthesis_nodes`, `build_jobs`) are deleted. `ScenarioSpec` is now `seed` + `difficulty` (tier ceiling only) + `steps` + `max_secs`. Machine positions are auto-assigned (a lane per machine type) — the runner is about *what/when*, not *where*.

**Rationale:** The runner exists to **tune progression, tech-tree gating, and recipe balance** by editing data, but the old format only exposed research-target and build lists — the entire machine buildout, config-module installs, recon timing, and terminal sequence were hardcoded, so re-sequencing any of that meant editing Rust. A prescriptive step list puts the whole run in the `.ron`: a balancing pass reorders `Research`/`Deploy`/`Build` steps directly. The smoke-test milestone flags were decoupled at the same time — `RunReport::observe_flags` sticky-observes them from world state each frame, so the data-driven run needn't assign them. Both e2e paths (`tests/standard_full_run.rs` Standard, `tests/initiation_run.rs` Initiation) now drive `Scenario::run` over their `.ron`, so the tests prove exactly what the balancing binary exercises.

**Notes / tradeoff:** A prescriptive run is **slower in virtual time** than the old parallel pump — each `Research(node)` blocks until that one node's currency accrues, sequentially, whereas the deleted pump requested all target nodes at once and unlocked opportunistically (Standard ≈ 2.68h vs the old ≈ 0.83h; Initiation ≈ 1.56h). Well within the test's `[100s, 24h)` bound, and the readability/tunability of an explicit ordered list is worth it. Three interpreter idioms follow from the tech-graph unlock mechanisms: `ExplorationDiscovery` nodes auto-unlock the frame their `Recon` fires (no `Research` step), `ProductionMilestone` nodes are a `Research`-as-wait, and a multi-recipe item (e.g. the minimal launch site) uses `Ensure(recipe)` + `Place`. Author guide: `testing.md §3`.

---

## 2026-07-13 — Logistics segmentation nodes renamed to "Sub-network Router I/II"

**Decision:** The two logistics segmentation tech nodes are renamed: "Logistics Controller" → **Sub-network Router I** (T3, first segmentation node) and "Sub-network Router" → **Sub-network Router II** (T4, deeper segmentation, strict behavioral superset of I). The deeper node being a superset, this uses the internal tier convention (superset behavior → I/II naming); display names may be re-flavored later.

**Rationale:** "Controller" carries the wrong connotation for players coming from AE2 (modded Minecraft), where a *controller* is what *emits* channels. In Exergon's design these nodes do **not** emit channels — channels come from cable tier — the node is a **segmentation boundary** (the IRL network-router concept: it partitions a network into sub-nets, each with its own channel budget). "Router" names that behavior correctly and avoids the false AE2 mental model. Prose describing the boundary as "controller/interface" is updated to "router/interface" for the same reason (`networks.md §5`, the 2026-07-12 channel-capacity entry).

**Notes:** Docs-only rename — these nodes are unauthored/unimplemented; no RON tech asset or `src/` symbol carries these names (confirmed). Unrelated "controller"s (the crafted Engineering `controller` assembly item, drone/body `Controller` code, power-network) are untouched.

---

## 2026-07-12 — Logistics throughput lever = discrete channel capacity (per-port), not flow rate

**Decision:** The logistics item-network gets the throughput lever the GDD already promises — **discrete channel capacity, AE2-style** (GDD §10) — and it is **per connected machine port**: each logistics port a machine joins to a network consumes **1 channel**, a network's channel budget is the channel rating of its cable tier, and ports beyond the budget are not served until the player lays higher-tier cable or **segments** into sub-networks joined by router/interface boundaries. This is explicitly **not** a Factorio-style per-tick items/second flow rate. Resolves the open GDD-§10-vs-`networks.md` divergence flagged in the 2026-07-10 config-dedication entry (obs 2962): the GDD specified channel capacity for logistics, but `networks.md §2` modelled *no* capacity (an uncapped priority-ordered shared pool via `has_items`/`take_items`/`give_items`), so the intended segmentation friction never existed in the technical model or `src/logistics/items.rs` — a primary reason run execution is "free" on the item side.

**Rationale:** The instant shared pool means one network JIT-serves every machine with zero routing cost, so the item-flow pillar has no design pressure. The GDD (§10, `gdd.md:434`) already names the fix — discrete channel limits that "encourage players to segment their network into logical sub-networks… segmentation is a solution to be discovered, not a forced constraint" — and the upgrade nodes are already designed but unauthored (`tech-tree-design.md` Logistics category: Channel Upgrade → Sub-network Router I → Sub-network Router II). Per-port-per-channel is the simplest AE2-faithful unit and mirrors the power network's existing amp model exactly: `LogisticsNetworkMembers` gains `channel_capacity()` / `channels_in_use()` and gates at the same point `recipe_start_system` already gates power on `amp_capacity()`/`amps_in_use()` (`networks.md §3`). Friction lives in **network topology** (a planning/design decision), not the runtime clock — Pillar-2 intact. This is the **item-flow** layer; it is complementary to, not a substitute for, machine-count scale (config-dedication, 2026-07-10) and autocraft-depth scale (CPU complexity-ceiling, below) — all three are planning/build friction, none slows a recipe.

**Alternatives considered:**
- *Per-tick items/second flow rate (Factorio belt-rate model):* Rejected — already rejected as the machine-scale lever (2026-07-10, `:59`), reintroduces the watch-and-fix/belt-throughput model the ME-network decision (GDD §10) deliberately cut, and meters transport speed (a runtime-clock friction) rather than network architecture. A content-design pass initially drafted the cable ladder in items/second; that unit is discarded in favor of channel counts.
- *Leave logistics uncapped (status quo):* Rejected — it is the divergence itself; the item-flow pillar stays frictionless and execution stays free.
- *Channel = per-item-type or per-sub-network (coarser units):* Deferred, not chosen — per-port is the faithful AE2 unit and the finest-grained forcing function for segmentation; revisit only if per-port proves too punishing in playtest.

**Implications:**
- **`networks.md §2`:** add a logistics channel-capacity model mirroring §3 power — `LogisticsNetworkMembers.channel_capacity()` / `channels_in_use()`, the per-port consumption rule, cable-tier channel ratings, and the non-destructive over-budget behavior (ports past the budget go unserved, no cable/machine damage), plus segmentation via router/interface boundaries.
- **Content:** the Channel Upgrade / Sub-network Router I / Sub-network Router II nodes (`tech-tree-design.md` Logistics category) get authored with **channel-count** ratings per cable tier (numbers TBD in a content pass), not flow rates. New cable-tier items gate on Engineering research.
- **Engine (Demo-scope, sequenced after the VS playtest gate):** `src/logistics/items.rs` shared-pool access becomes channel-gated; `recipe_start_system` (`networks.md §2`, input/output checks) gains a channel-availability check alongside the existing item checks.
- **CPU complexity-ceiling (System B, corrected):** the crafting-CPU idea survives as an autocraft **depth** cap (a plan may schedule at most N *new* craft steps; raw/stocked intermediates don't count), forcing standing intermediate stock — planning friction, Pillar-2 safe. Its "job-slots serialize the finale" half (a runtime time-sink) is dropped.
- No code yet — direction + mechanism lock. Closes obs 2962.
