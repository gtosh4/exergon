...
>
> Source of truth: [`tech-tree-design.md`](tech-tree-design.md) (§3 tiers, §6bis authored RON nodes). Escape ECS in [`technical/escape-condition.md §7.1`](technical/escape-condition.md); research themes in [`technical/research.md`](technical/research.md); balance methodology + flags in [`balance.md`](balance.md). Numbers throughout are representative and **unvalidated** — playtest-tuned after e2e completability (`balance.md §5 Q1`). *(The former `standard-run-design.md` was dissolved 2026-07-14; the "§N / §9 #N" shorthand in the phases below refers to its old sections — see the migration record in `design-decisions.md`.)*
...
- [x] **`wire_drawer` machine buildability audit** — RESOLVED. Placement is confirmed a separate path: `machine::registry` builds `PlaceableRegistry` from `assets/placeables/` **plus every machine def** (`FullMachineDef::to_placeable_def`), so any machine with an `assets/machines/*.ron` def is auto-placeable. Closed the loop for the forming machines by authoring `make_{crusher,washer,plate_roller,wire_drawer,refinery,storage_crate}` recipes (output the machine item on the tier-1 assembler) and wiring `UnlockMachine` + `UnlockRecipes([make_*])` onto their nodes (ore_crusher, ore_washer, plate_roller, basic_processing, silicon_refining, science_basics). Dead `draw_copper` id was already fixed → `draw_metal__copper`.
